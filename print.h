...
namespace _print_details
{
    ...
    template <class T, class U>
    struct _enable_if_glmvec<T, U, std::enable_if_t<std::is_same_v<decltype(T()[0] * T()), T> && std::is_same_v<decltype(T() * T()[0]), T> && std::is_same_v<decltype(T().x + T()), T> && std::is_same_v<decltype(T().x), typename T::value_type> && std::is_same_v<decltype(T()[0]), typename T::value_type &> && std::is_same_v<decltype(T() == T()), bool> && std::is_same_v<decltype(T(T()[0])), T> && std::is_same_v<decltype(T().length()), typename T::length_type> && std::is_same_v<decltype(std::declval<typename T::bool_type>().x), bool>>>
    {
        using type = U;
    };
    ...
    template <class T, class U>
    struct _enable_if_glmmat<T, U, typename _enable_if_glmvec<typename T::col_type, typename _enable_if_glmvec<typename T::row_type, std::enable_if_t<std::is_same_v<decltype(T()[0] * T()), typename T::row_type> && std::is_same_v<decltype(T() * T()[0]), typename T::col_type> && std::is_same_v<decltype(T()[0][0] * T()), T> && std::is_same_v<decltype(T()[0][0]), typename T::value_type &>>>::type>::type>
    {
        using type = U;
    };
    ...
    struct _printer<T, typename _enable_if_has_print<T, typename _enable_if_tuple<T, typename _enable_if_iterable<T>::not_type>::type>::not_type>
    {
        ...
        static void _unrolled_print(std::ostream &os, T const &t, std::index_sequence<Is...>)
        {
            ...
            if constexpr (sizeof...(Is) != 0)
                _printer<_rmcvref_t<std::tuple_element_t<sizeof...(Is), T>>>::print(os, std::get<sizeof...(Is)>(t));
            os << "}";
        }
    ...
    struct _printer<T, typename _enable_if_has_print<T, typename _enable_if_variant<T>::type>::not_type>
    {
        static void print(std::ostream &os, T const &t)
        {
            std::visit([&](auto const &v)
                       { _printer<_rmcvref_t<decltype(v)>>::print(os, v); }, t);
        }
    };
    ...
    public:
        ...
        friend std::ostream &operator<<(std::ostream &os, print_adaptor const &&self)
        {
            ...
            os << "[object 0x" << std::hex << reinterpret_cast<std::uintptr_t>(reinterpret_cast<void const volatile *>(std::addressof(self.t))) << ']';
            os.flags(oldflags);
            return os;
        }
    };
